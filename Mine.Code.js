...
function handleMouseEvent(event) {
    ...
    if (event.button === 0) {
        //don't response to "expanded" and "marked" case
        if (oMine.getAttribute("marked") === K_TRUE || oMine.getAttribute("expanded") === K_TRUE) {
            return false;
        }
...
function handleLeftClick(oMine) {
    oMine.setAttribute("pushed", false);
    var mine_index = parseInt(oMine.getAttribute("mine_index"), 10);
    if (!is_first_click) {
        is_first_click = true;
        InitMineArea(row_count, col_count, mine_count, mine_index);
    }
    ExpandMineArea(mine_index);
    CheckGameStatus();
}
